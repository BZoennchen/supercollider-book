...
Through the rigid coupling of air molecules (medium) sound (form) is constructed (see [Medium and Form](sec-medium-and-form)).
...
Our synthesizer-code, and what the languages provides us with to use this code, builds up a new medium.
...
Within the language, we can use plain iteration and threading via [Routines](https://doc.sccode.org/Classes/Routine.html) and [Tasks](https://doc.sccode.org/Classes/Task.html), see section [Routines and Tasks](sec-routines-tasks).
...
Using patterns requires you to learn a new vocabulary and until one has a critical mass of knowledge about the different patterns, it can be hard to trust them.
