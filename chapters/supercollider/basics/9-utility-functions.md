...
Especially if we define a [synth definition](sec-synths), we want to deal with frequencies.
...
To use decibels instead of amplitude, where 0 decibels is equivalent to 1.0 amplitude, we can make use of the built-in functions ``x.ampdb`` and ``y.dbamp``.
``x.ampdb`` converts a loudness value in amplitude into decibels.
...
Transforming amplitude to decibels and vice versa is an application of a mapping that maps one range of values into another.
...
+ ``explin`` maps an exponential function to a linear function
+ ``expexp`` maps an exponential function to another exponential function
+ ``lincurve`` like ``linexp`` but you can define the steepness of the exponential curve
+ ``curvelin`` like ``explin`` but you can define the steepness of the exponential curve
...
``lincurve`` and ``curvelin`` give us a little bit more control over the shape of the transformation.
...
For example, the following code gives us [midi notes](sec-midi-notes) which belong to the [C major scale](sec-diatonic-scale).
...
On the **server-side** there are corresponding unit generators such as
...
Instead of dealing with abstract filter arguments, it lets you conveniently define the duration of the transition.
...
To post the numbers a unit generator produces to the post window you can use ``poll``.
...
Furthermore, we convert -7 decibels into amplitude and make use of ``k.reciprocal`` which computes ``1/k``.

The sound is generated by three sine waves at frequency ``\freq``, ``\freq`` + 0.01 semitones and ``\freq`` - 0.01 semitones.
...
Let us use two sine waves changing their frequency 5 times a second via a smooth transition.
