...
After connecting my device to my computer using an appropriate USB cable and waiting for round about 40 seconds I am ready to go.
...
Bela replaces the IDE with a browser based IDE which you can access after connecting by typing ``http://bela.local/`` into your browser.
Furthermore, **scsynth** is running on the server in real-time and this can be controlled by analog and digital sensors connected to the system.

Because the SuperCollider architecture consists of an audio server and a client, there are two possible modes we can use the Bela device.
...
Therefore, we establish the communication between our laptop and the Bela device via [OSC](sec-osc).
