...
Delays are the building blocks for two other central concepts in digital signal processing:
...
seconds. 
For an oscillator with a frequency of $100$ Hz, a phase shift of $\pi$ would imply a delay of only $0.005$ seconds.
...
In fact, the amplitude of each even harmonic is reinforced since
...
This is much more convenient since it is independent of the sample rate which depends on the system.
...
With [DelayL](https://doc.sccode.org/Classes/DelayL.html) we can achieve the filtering of the sawtooth wave without combining the sine waves by using multiple [UGens](def-ugen).
...
Therefore, a *feedforward comb filter* is a *filter*!
...
A *feedforward comb filter* is also a [finite impulse response filter](def-fir-filter).
...
If the *feedback coefficient* $\beta < 1.0$ consecutive peaks decrease (stable network), if $\beta > 1.0$ (unstable) they increase, and if $\beta = 1$ (also considered as unstable) they stay at a constant amplitude.

Again, the comb filter amplifies/attenuate certain frequencies.
Therefore, a *feedback comb filter* is a *filter*!
A *feedback comb filter* is an [infinite impulse response filter](def-iir-filter).
